...
void cloud_grasps(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_in, std::vector<Eigen::Vector3d> clustered_grasps){
  for (int i = 0; i < clustered_grasps.size(); i++){
    pcl::PointXYZ point;
    point.x = clustered_grasps[i][0];
    point.y = clustered_grasps[i][1];
    point.z = clustered_grasps[i][2];

    cloud_in->push_back(point);
  }
}

// function to cluster grasps based on euclidean distance
std::vector<Eigen::Vector3d> grasp_cluster(std::vector<Eigen::Vector3d> grasps){
  pcl::PointCloud<pcl::PointXYZ>::Ptr temp_cloud (new pcl::PointCloud<pcl::PointXYZ>);

  temp_cloud->width = grasps.size();
  temp_cloud->height = 1;
  temp_cloud->points.resize(temp_cloud->width * temp_cloud->height);

  for (int i = 0; i < grasps.size(); i++){
    temp_cloud->points[i].x = grasps[i][0];
    temp_cloud->points[i].y = grasps[i][1];
    temp_cloud->points[i].z = grasps[i][2];
  }
  ...
  ec.setClusterTolerance(0.05); //Euclidean distance between points for a cluster
  ec.setSearchMethod(tree);
  ec.setInputCloud(temp_cloud);
  ec.extract(cluster_indices);

  std::vector<Eigen::Vector3d> clustered_grasps; 
  std::vector<int> index_track;
  Eigen::Vector3d vector_point;

  for (std::vector<pcl::PointIndices>::const_iterator it = cluster_indices.begin(); it != cluster_indices.end(); ++it){
    for (std::vector<int>::const_iterator pit = it->indices.begin(); pit != it->indices.end(); ++pit){
      pcl::PointXYZ point = temp_cloud->points[*pit];
      vector_point[0] = point.x;
      vector_point[1] = point.y;
      vector_point[2] = point.z;

      clustered_grasps.push_back(vector_point);
      break; //Only one point pushed per cluster
    }
  }

  return clustered_grasps;
...
int main(int argc, char* argv[])
{
  ...
  // Save the cloud centroid into a file
  pcl::PointCloud<pcl::PointXYZRGBA>::Ptr cloud_original (new pcl::PointCloud<pcl::PointXYZRGBA>);
  Eigen::Vector4f centroid;
  cloud_original = cloud_cam.getCloudOriginal();
  pcl::compute3DCentroid(*cloud_original, centroid);
  ...
  // Save the bottom of the finger grasps to a file
  //std::string grasp_filename = argv[2];
  std::string grasp_filename = "grasp_poses.txt";
  std::ofstream myfile;
  myfile.open (grasp_filename.c_str());
  myfile << "Centroid: " << centroid[0] << ", " << centroid[1] << ", " << centroid[2] << "\n";
  std::vector<Eigen::Vector3d> clustered_grasps;
  std::vector<Eigen::Vector3d> unclustered_grasps;
  Eigen::Vector3d Bottom;

  /*for (int i = 0; i < candidates.size(); i++)
  {
    Eigen::Vector3d Bottom = candidates[i].getGraspBottom();
    Eigen::Vector3d Top = candidates[i].getGraspTop();

    myfile << Bottom[0] << ", " << Bottom[1] << ", " << Bottom[2] << "\n";
    myfile << Top[0] << ", " << Top[1] << ", " << Top[2] << ";" << "\n";
  ...
  if (pcl::io::loadPLYFile(argv[2], *cloud_input) == -1){
    PCL_ERROR("Couldn't load the ply file");
  }

  if (candidates.size() > 0){
    for (int i = 0; i < candidates.size(); i++){
      unclustered_grasps.push_back(candidates[i].getGraspBottom());
    }
    clustered_grasps = grasp_cluster(unclustered_grasps);
    for (int i = 0; i < clustered_grasps.size(); i++){
      Bottom = clustered_grasps[i];
      myfile << Bottom[0] << ", " << Bottom[1] << ", " << Bottom[2] << "\n";
    }

    cloud_grasps(cloud_input, clustered_grasps);
    pcl::io::savePLYFileASCII("new_cloud.ply", *cloud_input);
  } 
