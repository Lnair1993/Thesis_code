...
function [ranked_score_new, action_part_idx, grasp_part_idx, action_att_points, grasp_att_points] = MG_lvl_1(input_full, input_action, input_grasp)
...
input_full_tool_file = input_full;
input_tool_action_file = input_action;
input_tool_grasp_file = input_grasp;
...
[action_params, action_fit_type, ~, ~] = SQ_fitting_params(input_tool_action, SQ_type_action);
%action_res;
...
[grasp_params, grasp_fit_type, ~, ~] = SQ_fitting_params(input_tool_grasp, SQ_type_grasp);
%grasp_res;
...
for i = 1:length(file_list) % Read each segment file
    ...
    [point_cloud, ~] = tool_scaling(tool_cat, point_cloud);
    
    
    [action_SQ_params, ~, ~, residue_action] = SQ_fitting_params(point_cloud, SQ_type_action);    
    score_action = residue_action;
    
 
    [grasp_SQ_params, ~, ~, residue_grasp] = SQ_fitting_params(point_cloud, SQ_type_grasp);    
...
% Return final fitting scores, ranked action parts, ranked grasp parts,
% corresponding action_att_IDs, grasp_att_IDs
