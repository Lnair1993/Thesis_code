...
function [ranked_score_new, action_part_idx, grasp_part_idx, action_att_points, grasp_att_points] = att_finder(action_scores, grasp_scores, parts_files_list, part_location, input_full_tool_file, action_scale, grasp_scale, ranked_action_params, ranked_grasp_params)
    ...
    %fprintf('Ranked scoring of parts: ');
    %ranked_score_new
    %action_part_idx
    %grasp_part_idx
...
%         end
        ...
        ranked_score_new(i) = ranked_score_new(i) + action_dist + grasp_dist; %Based on att points
    end
    
    [~,ranked_idx] = sort(ranked_score_new); %Sort in ascending order (low score better)
    ...
    %fprintf("Internally evaluated objects: ");
    %ranked_score_new
    %action_part_idx
    %grasp_part_idx
    
    %Pick best ranked part
    %action_match = parts_files_list(action_part_idx(3),:).name;
    %grasp_match = parts_files_list(grasp_part_idx(3),:).name;
        
    % Orient them based on the input/source tool
    %[action_pcl_tf, grasp_pcl_tf, ~, ~] = parts_orient(action_match, grasp_match, angle_relative, pos_relative, part_location);
    ...
    %att_point_IDs = [action_att_points(1), grasp_att_points(1)] 
