...
function [action_pcl_tf, grasp_pcl_tf, min_grasp_x, max_action_x, att_point_action, att_point_grasp] = parts_orient2(action_match, grasp_match, angle_relative, pos_relative, part_location)
    ...
    %Append homogeneous coordinates 1
    att_point_action = vertcat(att_point_action, ones(1,size(att_point_action,2)));
    att_point_grasp = vertcat(att_point_grasp, ones(1,size(att_point_grasp,2)));
    ...
    %hold on;
    %scatter3(att_point_action(:,1), att_point_action(:,2), att_point_action(:,3));
    %hold on;
    %scatter3(att_point_grasp(:,1), att_point_grasp(:,2), att_point_grasp(:,3));
