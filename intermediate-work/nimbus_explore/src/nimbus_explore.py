...
# Implement system pipeline

# ---------- Section 1 ----------
# Use functions from pierce_predict.py

# ---------- Section 2 ----------
# Use new info for computing final ranking -- MATLAB

# ---------- Section 3 ----------
# System takes in object final rankings 
# Define a function for performing grasping
# Define a function for performing piercing
# Depending on outcome, re-order ranking
# For any object for which properties unknown, repeat section 1

# NOTE: All robot moving stuff is here 

# MATLAB returns ranking with object names, geometric score, corresponding attachment types, grasp and magnet scores for relevant ones (None for others); also take as input object positions
...
def ESF_retrieve(point_cloud): # CHECK THIS ONE
	ESF = {}
	ESF_file = 'esf_descriptors_full.csv'
...
def geoscore_predict(obj_1, obj_2, reference_action, learning_file):
	...
	joblib_location = '/home/lnair3/Nimbus_ws/src/nimbus_explore/src/NN_joblib_files/'
...
def output_reader(output_file, reference_action=None, learning_file=None):
    # Read the MATLAB output and parse it into a dict with keys corresponding to object tuples, values corresponding to a dict of geometric_score, attachment_type, att_score
    #{(obj_1, obj_2):{geo_score:X, att_type:A, att_score:Y}
    #{(obj_1, obj_2):rank}
    object_rank = []
    objects_attr = {}
    objects_list = []
    ...
    with open(output_file) as f:
        ...
        for idx, row in enumerate(reader):
            ...
            if learning_file is None:
            	...
            	geoscore = geoscore_predict(obj_1, obj_2, reference_action, learning_file)
    ...
    '''if learning_file is not None:
    	# Take the ESF dict
    	# Pass ESF to the NN - get prediction probability
    	# Product of prediction probability saved to the objects_attr
        ...
        with open(learning_file) as f:
            ...
            for idx, row in enumerate(reader):
                ...
                obj_2 = row[1]
                score = row[2]
                objects_attr[((obj_1, obj_2), idx)]['geoscore'] = -score # Negative because higher probability is better
                objects_attr[((obj_1, obj_2), idx)]['total_score'] = -score'''
    
    return object_rank, objects_attr

def rank2csv(obj_rank, obj_attr):
    data = {}
    final_dict = {}
    field_names = ['obj_combo', 'geoscore', 'att_type', 'att_score', 'material_score', 'total_score']

    with open('final_rank.csv', 'w') as f:
        writer = csv.DictWriter(f, fieldnames = field_names)
        writer.writeheader()
        for elt in obj_rank:
            final_dict = obj_attr[elt[0]]
            data['obj_combo'] = elt[0][0]
...
def object_reranking(objects_attr, MG_objs_list, att_types_used, learning=False, user_rank=None):
    ...
    materials_wt = 0.0 # weighting associated with materials match

    materials_score = [0.0, 0.25, 0.5, 1.0]

    att_wt = 1.0 # to rank based on attachment score only
    ...
    for pairs in objects_attr_new:
        ...
        # ADD SCORE CORRESPONDING TO MATERIALS MATCH
        if user_rank is not None:
            for idx, part in enumerate(user_rank):
                mg_obj = MG_objs_list[pairs[0][idx]]
                obj_material = mg_obj.material_class
                if obj_material == None:
                    print "Unknown material class"

                user_idx = part.index(obj_material)  
                mat_score = materials_score[user_idx]
...
def update_attr(obj_attr, obj_poses, key, reference_action):
    ...
    if mg_obj.pierce_predict is None:
        ...
        scio_features = object_sense(key, obj_poses[key])
...
def object_sense(obj_name, obj_pose):
    # Ideally, robot moves sensor over obj_pose and scans, retrieves the features
    # Currently, read all features from MG_dataset and then only return feature corresponding to obj_name
    MG_dataset = 'Macgyver_DS_full.csv' # Should be MG_DS_all_objects.csv
...
#user_rank = [['metal', 'wood', 'plastic', 'foam'], ['wood', 'metal', 'plastic', 'foam']] # Example material preference of user [[action_part], [grasp_part]]
user_rank = None
construct_tool = False # Flag on whether to proceed with actual tool construction

# Remove this when robot scanning them individually
...
#gs = geoscore_predict(obj_1, obj_2, reference_action, learning_file)
#print gs

obj_rank, obj_attr = output_reader(output_file, reference_action, learning_file)
obj_poses = object_pose_reader(poses_file)

MG_objs_list = {}

# CHANGE THIS TO ONLY THE TOP RANKED OBJECTS - CHANGE TO JUST OBJECTS IN OUTPUT_FILE
for key in obj_poses.keys():   
    MG_objs_list[key] = update_attr(obj_attr, obj_poses, key, reference_action)

# Re-rank objects 
new_obj_rank, obj_attr_new = object_reranking(obj_attr, MG_objs_list, att_types_used, user_rank)
...
if construct_tool:
    ...
    while not working_tool:
        ...
        if attach_fail_flag:
            ...
            new_obj_rank, obj_attr_new = object_reranking(obj_attr_new, MG_objs_list, att_types_used, user_rank)
