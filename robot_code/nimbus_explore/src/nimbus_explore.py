...
def ESF_retrieve(point_cloud): # CHECK THIS ONE
	ESF = {}
	ESF_file = 'esf_descriptors_final_DS.csv'
...
def rank2csv(obj_rank, obj_attr, final_file_loc = None):
    ...
    with open(final_rank_file, 'w') as f:
        ...
        writer.writeheader()
        for elt in obj_rank:
            final_dict = {'geoscore':0, 'att_type':0, 'att_score':0, 'material_score':0, 'total_score':0}
...
def object_reranking(objects_attr, MG_objs_list, att_types_used, dual_NN_embeddings=None, dual_NN_wts=None, dual_NN_scaler=None, normalize=False, user_rank=None):
    ...
    magnetic_wt = -1.0 #-1.0 # weighting associated with magnetic attachment
    grasp_wt = -1.0 #-1.0 # weighting associated with grasp attachment
    materials_wt = 1.0 #1.0 # weighting associated with materials match

    materials_score = [0.0, 0.25, 0.5, 1.0]

    att_wt = 1.0 #1.0 # to rank based on attachment score only
    ...
    for pairs in objects_attr_new:
        ...
        elif dual_NN_embeddings is not None: ## Use Dual NN
        	obj_name = pairs[0][0]
...
main_folder_loc = '/home/lnair3/Output_ranking_final_DS_random'
