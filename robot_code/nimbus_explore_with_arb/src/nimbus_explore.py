...
def arbitration(objects_attr, reference_action, arbitration_type):
    for elt in objects_attr:
    	if arbitration_type == 'normalize':
    		if len(elt[0]) == 1:
    			obj_name = elt[0][0]
    			subs_file = 'substitution_ESF.csv'
    			obj_esf = ESF_retrieve(elt[0][0], subs_file)
    			geo_score = shape_dualNN(obj_esf, reference_action)
    			objects_attr[elt]['geoscore'] = geo_score

    	elif arbitration_type == 'dual_score':
    		if len(elt[0]) == 1:
    			obj_name = elt[0][0]
    			esf_file = 'substitution_ESF.csv'
    		else:
    			obj_1 = elt[0][0]
    			obj_2 = elt[0][1]
    			esf_file = 'esf_descriptors_constructions.csv'
    			obj_name = obj_1 + '_JOIN_' + obj_2 + '.ply'
    		obj_esf = ESF_retrieve(obj_name, esf_file)
    		geo_score = shape_dualNN(obj_esf, reference_action)
    		objects_attr[elt]['geoscore'] = geo_score

    	elif arbitration_type == 'rule':
    		if len(elt[0]) == 1 and objects_attr[elt] >= 1.0:
    			objects_attr[elt]['geoscore'] = 0
    		elif len(elt[0]) == 1 and objects_attr[elt] < 1.0:
    			objects_attr[elt]['geoscore'] = -float('inf')
    		elif len(elt[0]) > 1 and objects_attr[elt] >= 1.0:
    			objects_attr[elt]['geoscore'] = -10
    			objects_attr[elt]['att_type'] = 'subs'
    			objects_attr[elt]['att_score'] = 0
    		else:
    			objects_attr[elt]['geoscore'] = -float('inf')
    			objects_attr[elt]['att_type'] = 'subs'
    			objects_attr[elt]['att_score'] = 0
    		...
    		print("ERROR: Invalid Arbitration type")
...
main_folder_loc = '/home/lnair3/Output_ranking_final_DS_ShapeAttMat' #'/home/lnair3/Output_ranking_final_DS' 
