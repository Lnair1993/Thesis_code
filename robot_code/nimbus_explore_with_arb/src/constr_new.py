...
def output_reader(output_file, reference_action=None, learning_file=None, constr_file=None, arbitration_type=None):
    ...
    with open(output_file) as f:
        ...
        for idx, row in enumerate(reader):
            ...
            if learning_file is None:
            	...
            	geo_score = 0 #geoscore_predict(obj_1, obj_2, reference_action, learning_file)
            	#constr_name = obj_1 + '_JOIN_' + obj_2 + '.ply'
            	#constr_esf = ESF_retrieve(constr_name, constr_file)
                #geo_score = shape_dualNN(constr_esf, reference_action)
...
def object_reranking(objects_attr, MG_objs_list, att_types_used, dual_NN_embeddings=None, dual_NN_wts=None, dual_NN_scaler=None, normalize=False, user_rank=None):
    ...
    for pairs in objects_attr_new:
        if 'pierce' in objects_attr_new[pairs]['att_type'] and 'pierce' in att_types_used:
            ...
            objects_attr_new[pairs]['total_score'] = 0 # Substitutes have no attachments

        # ADD SCORE CORRESPONDING TO MATERIALS MATCH - Different scores based on rank
        ...
        # ADD SCORE CORRESPONDING TO MATERIALS MATCH - Fixed score for all appropriate materials    
        #if user_rank is not None:
        #	mg_obj = MG_objs_list[pairs[0][0]]
        #	obj_material = mg_obj.material_class
        #	if obj_material == None:
        #		print "Unknown material class"
#
        #	if obj_material in user_rank:
        #		mat_score = 0.1
        #		objects_attr_new[pairs]['material_score'] = mat_score
        #	else:
        #		mat_score = -float('inf')
...
main_folder_loc = '/home/lnair3/Output_ranking_final_DS_Att' #'/home/lnair3/Output_ranking_final_DS' 
tool_folders = ['Flip', 'Hit', 'Rake', 'Scoop', 'Screw', 'Squeegee'] # FLIP
num_sub_cases = 10
...
for tool_folder in tool_folders:
	for folder_num in range(0,num_sub_cases):
		...
		constr_file = '/home/lnair3/Dataset_constr/' + tool_folder + '/' + str(folder_num+1) + '/' + 'esf_descriptors_constructions.csv'

		obj_rank, obj_attr, obj_list = output_reader(output_file, reference_action, learning_file, constr_file)
